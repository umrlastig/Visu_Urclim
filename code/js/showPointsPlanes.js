...
export function handleCreationPlane(id, remove = true) {
  if (id.split("_")[0] === "SBL") {
    if (
      general_config.id_sbl_array_real_plane.includes(
        parseInt(id.split("_")[1])
      )
    ) {
      general_config.id_sbl_array_real_plane = removeValue(
    ...
    if (
      general_config.id_meso_array_real_plane.includes(
        parseInt(id.split("_")[1])
      )
    ) {
      general_config.id_meso_array_real_plane = removeValue(
...
/**
 * function that allows to choose between the 3 creations points methods depending on the type of points chosen by the user
 */
export function chooseCreationPoints() {
  let ckboxes = document.querySelectorAll(".type_de_points");
  let typePts;
  ckboxes.forEach((box) => {
    if (box.checked == true) {
...
function handleCreationRandomPoints(id) {
  if (id.split("_")[0] === "SBL") {
    if (general_config.id_sbl_array.includes(parseInt(id.split("_")[1]))) {
      general_config.id_sbl_array = removeValue(
        general_config.id_sbl_array,
        parseInt(id.split("_")[1])
      );
    } else general_config.id_sbl_array.push(parseInt(id.split("_")[1]));
  } else {
    if (general_config.id_meso_array.includes(parseInt(id.split("_")[1]))) {
      general_config.id_meso_array = removeValue(
        general_config.id_meso_array,
        parseInt(id.split("_")[1])
      );
    } else general_config.id_meso_array.push(parseInt(id.split("_")[1]));
  }
...
export function handleCreationVerticalPlane(id) {
  if (id.split("_")[0] === "SBL") {
    if (
      general_config.id_sbl_array_vertical_plane.includes(
        parseInt(id.split("_")[1])
      )
    ) {
      general_config.id_sbl_array_vertical_plane = removeValue(
        general_config.id_sbl_array_vertical_plane,
        parseInt(id.split("_")[1])
      );
    } else
      general_config.id_sbl_array_vertical_plane.push(
        parseInt(id.split("_")[1])
      );
  } else {
    if (
      general_config.id_meso_array_vertical_plane.includes(
        parseInt(id.split("_")[1])
      )
    ) {
      general_config.id_meso_array_vertical_plane = removeValue(
        general_config.id_meso_array_vertical_plane,
        parseInt(id.split("_")[1])
      );
    } else
      general_config.id_meso_array_vertical_plane.push(
        parseInt(id.split("_")[1])
      );
  }
